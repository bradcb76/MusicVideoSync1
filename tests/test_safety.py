...
def test_compose_keeps_music_read_only():
    ...
    assert "PROCESSING_PATH: /processing" in compose
    assert "STAGING_PATH: /staging" in compose
    assert '"mvs-processing:/processing"' in compose
    assert '"mvs-staging:/staging"' in compose
